...
//----- (1000AF00) --------------------------------------------------------
HRESULT __thiscall flipSurfaces(unk334 *this) {
    unk5C *v2;                             // eax
    unk5C *v3;                             // edi
    ...
    this->     ?->lpVtbl->Clear(this->?, 1, &rect, D3DCLEAR_ZBUFFER | D3DCLEAR_TARGET);
    hr = this->?->lpVtbl->Flip(this->?, this->?, DDFLIP_WAIT);
  while ( this->?->lpVtbl->GetFlipStatus(this->?, DDGFS_ISFLIPDONE) )
      ;
  return hr;
}
