...
bool __cdecl sub_10070AF0(char *filename, BOOL b) {
    ...
    if (strcmpResult)
        goto LABEL_19;
    if (freadLock(&buffer, 4u, 1u, fp1) != 1)
        goto LABEL_18;
    if ((unsigned int)buffer < 0xC) {
        fp2 = fp1;
        if (!buffer) {
            if (freadLock(&filename, 1u, 4u, fp1) == 4)
                goto LABEL_9;
        LABEL_18:
            fp2 = fp1;
        }
...
LABEL_9:
    filename -= 4;
    if (freadLock(&v21, 1u, 4u, fp1) != 4)
        goto LABEL_18;
    if ((unsigned int)buffer < 0xC) {
        filename -= 4;
        if (freadLock(&dword_10AA28CC, 1u, 4u, fp1) != 4)
            goto LABEL_18;
        filename -= 4;
        if (freadLock(&dword_10AA28C8, 1u, 4u, fp1) != 4)
            goto LABEL_18;
    }
    filename -= 16;
    size = freadLock(dword_10680738, 1u, 0x10u, fp1);
    fp2 = fp1;
    if (size != 16)
        goto LABEL_19;
    v17 = filename;
    v6 = sub_1006ADF0();
    v7 = (char *)freadLock(v6, 1u, (size_t)v17, fp1);
    if (v7 != filename)
        goto LABEL_18;
    ...
    *((_WORD *)dword_10ACED34 + 120) |= 1 << SBYTE1(dword_10680738[0]);
    ...
    v15 = ftell(fp1);
    fseek(fp1, v15 - 152, 0);
    freadLock(&dword_10AD0978, 4u, 1u, fp1);
    freadLock(&dword_10AD097C, 4u, 1u, fp1);
    freadLock(&dword_10AD0980, 4u, 1u, fp1);
    freadLock(&dword_10AD0984, 4u, 1u, fp1);
    freadLock(&dword_10AD0988, 4u, 1u, fp1);
    freadLock(&dword_10AD098C, 4u, 1u, fp1);
    fseek(fp1, 0, 2);
    v16 = ftell(fp1);
    fseek(fp1, v16 - 128, 0);
    freadLock(gBuffer1, 1u, 0x80u, fp1);
