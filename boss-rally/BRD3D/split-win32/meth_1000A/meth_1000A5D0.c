...
int __thiscall meth_1000A5D0(unk0 *this) {
    ...
    if (width == 320 && height == 200 && bitDepth == 8)
        flags = 1;
    hr = this->ddraw2->lpVtbl->SetDisplayMode(
        this->ddraw2, width, height, bitDepth, refreshRate, flags);
    if (hr < 0 &&
        (width == 640 && height == 480 ||
         (iid = this->unk,
          ...
          (this->u0 = v10) == 0) ||
         (hr = this->ddraw2->lpVtbl->SetDisplayMode(this->ddraw2, width, height, bitDepth, 0, 0),
          hr < 0))) {
        if (bitDepth == 16 ||
            (iid0 = (int)this->unk,
