...
void __cdecl GetDirectXVersion(_DWORD *dxVersion, _DWORD *out_b) {
        ...
        *out_b = 2;
        ...
        if (result == 4) {
            *dxVersion = 512;
            ...
            if (dinputDllHmod) {
                DirectInputCreateA = (HRESULT(__stdcall *)(
                    HINSTANCE, DWORD, LPDIRECTINPUTA *, LPUNKNOWN))GetProcAddress(dinputDllHmod,
                                                                                  aDirectinputcre);
    ...
    if (drawDllHmod) {
        DirectDrawCreatePA = (HRESULT(__stdcall *)(
            GUID *, LPDIRECTDRAW *, IUnknown *))GetProcAddress(drawDllHmod, aDirectdrawcrea);
                ...
                *dxVersion = 256;
                if (ddrawVtbl->QueryInterface(lpDDraw, &CLSID_IDirectDraw2, &ppvDDraw2) >= 0) {
                    ...
                    *dxVersion = 512;
                    ...
                    if (dinputDllHmod_3) {
                        DirectInputCreatePA =
                            (HRESULT(__stdcall *)(HINSTANCE, DWORD, LPDIRECTINPUTA *, LPUNKNOWN))
                                GetProcAddress(dinputDllHmod_3, aDirectinputcre);
