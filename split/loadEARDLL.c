...
HMODULE __cdecl loadEARDLL(int useEarpdsDll)
{
  ...
  if ( result
    || useEarpdsDll
    || (dword_105754D4 = 0, dword_105754D8 = 1, result = GetModuleHandleA(aEarpdsDll), (hModule = result) != 0)
...
LABEL_12:
    if ( !EAR_DLL_UpdateEar )
    {
      ...
      EAR_DLL_ChangeChannelControl = (int (__stdcall *)(_DWORD, _DWORD))GetProcAddress(hModule, aEarDllChangech);
      EAR_DLL_ClearChannel = (int (__stdcall *)(_DWORD, _DWORD))GetProcAddress(hModule, aEarDllClearcha);
      ...
      EAR_DLL_SetAttenuationLevel = (int (__stdcall *)(_DWORD, _DWORD))GetProcAddress(hModule, aEarDllSetatten);
      ...
      EAR_DLL_ShutDownChannel = (int (__stdcall *)(_DWORD))GetProcAddress(hModule, aEarDllShutdown_0);
      ...
      if ( EAR_DLL_EarInactive == 0
        || EAR_DLL_GetEventStatus == 0
        || EAR_DLL_ChangeChannelControl == 0
        || EAR_DLL_ClearChannel == 0
        || EAR_DLL_AAA_Validate == 0
        || EAR_DLL_AssignHwnd == 0
        || EAR_DLL_ShutDownBank == 0
        || EAR_DLL_ShutDownChannel == 0
        || EAR_DLL_SetUserDistanceUnit == 0
        || EAR_DLL_ShowLastError == 0
        || EAR_DLL_ResetEar == 0
        || EAR_DLL_SetAttenuationLevel == 0
        || EAR_DLL_RegisterMatrix == 0
        || EAR_DLL_RegisterPreset == 0
        || EAR_DLL_RegisterChannel == 0
        || EAR_DLL_RegisterEnvironment == 0
        || EAR_DLL_MoveEvent == 0
        || EAR_DLL_RegisterBank == 0
        || EAR_DLL_InitializeEar == 0
        || EAR_DLL_MixEvent == 0
        || EAR_DLL_GetLastError == 0
        || EAR_DLL_GetVersion == 0
        || EAR_DLL_UpdateEar == 0
        || EAR_DLL_StartEvent == 0
        || EAR_DLL_StartTimer == 0
        || EAR_DLL_ShutDownPreset == 0
        || EAR_DLL_ShutDownTimer == 0
        || EAR_DLL_ShutDownEvent == 0
        || EAR_DLL_ShutDownMatrix == 0
        || EAR_DLL_ShutDownEar == 0
